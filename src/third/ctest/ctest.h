...
typedef struct ctest_test_global_t ctest_test_global_t;
...
// global init/destroy
struct ctest_test_global_t {
  ctest_test_func_pt *init_func;
  ctest_test_func_pt *destroy_func;
};
...
extern ctest_list_t      *ctest_init_list;
extern ctest_test_global_t ctest_test_global;
...
extern jmp_buf ctest_jmp_buf;

static int ctest_test_exec_func(ctest_test_func_pt *func) {
  if (NULL != func) {
    ctest_test_retval = 0;
    if (0 == setjmp(ctest_jmp_buf)) {
      func();
    } else {
      /*
    ...
    return ctest_test_retval;
  }
  return 0;
}

static int ctest_test_exec_case(ctest_test_case_t *tc, int *assert_flag)
{
    ctest_test_func_t        *t;
    int64_t                 t1, t2;
    int                     failcnt = 0;
    ...
    ctest_list_for_each_entry(t, &tc->list, listnode) {
        ...
        // failure
        if (ctest_test_retval > 0) {
            ctest_test_color_printf(CTEST_TEST_COLOR_RED, "[  FAILED  ]");
            failcnt ++;
        } else if (ctest_test_retval < 0) {
            ctest_test_color_printf(CTEST_TEST_COLOR_RED, "[  ABORT  ]");
            failcnt ++;
            if (assert_flag) {
              *assert_flag = 1;
              break;
...
static inline int ctest_test_main(int argc, char *argv[])
{
    ...
    init_destroy_ret = ctest_test_exec_func(ctest_test_global.init_func);
    if (init_destroy_ret > 0) {
        ctest_test_color_printf(CTEST_TEST_COLOR_RED, "[  INIT FAILED  ]\n");
        return 1;
    } else if (init_destroy_ret < 0) {
        ctest_test_color_printf(CTEST_TEST_COLOR_RED, "[  INIT ABORT  ]\n");
        return 1;
    }
    ...
    int assert_flag = 0;
    ctest_list_for_each_entry(tc, &ctest_test_case_list, listnode) {
        total_failcnt += ctest_test_exec_case(tc, &assert_flag);
        if (assert_flag) {
          break;
        }
    }
    ...
    init_destroy_ret = ctest_test_exec_func(ctest_test_global.destroy_func);
    if (init_destroy_ret > 0) {
        ctest_test_color_printf(CTEST_TEST_COLOR_RED, "[  DESTROY FAILED  ]\n");
    } else if (init_destroy_ret < 0) {
        ctest_test_color_printf(CTEST_TEST_COLOR_RED, "[  DESTROY ABORT  ]\n");
    }
...
#define CTEST_TEST_MAIN_DEFINE                                                \
    ...
    ctest_test_global_t ctest_test_global = {.init_func=NULL, .destroy_func=NULL, };  \
...
#define TEST_INIT(init_name)                                                  \
    void TEST_CASE(init_name, gsetup)();                                      \
    __attribute__((constructor)) void ctest_testd_##init_name##_gsetup() {    \
      ctest_test_global.init_func = TEST_CASE(init_name, gsetup);             \
    }                                                                         \
    void TEST_CASE(init_name, gsetup)()

#define TEST_DESTROY(destroy_name)                                            \
    void TEST_CASE(destroy_name, gdown)();                                    \
    __attribute__((constructor)) void ctest_testd_##destroy_name##_gdown() {  \
      ctest_test_global.destroy_func = TEST_CASE(destroy_name, gdown);        \
    }                                                                         \
    void TEST_CASE(destroy_name, gdown)()
...
#define __TEST_TRUE(c, ret)                                                   \
    if(!(c) && ctest_test_retval >= 0) {                                      \
      printf("ERROR at %s:%d, EXPECT_TRUE(" #c ")\n",                         \
                 __FILE__, __LINE__);                                         \
      ctest_test_retval = (ret);                                              \
      if ((ret) < 0) {                                                        \
        longjmp(ctest_jmp_buf, (ret));                                        \
      }                                                                       \
    }

#define __TEST_FALSE(c, ret)                                                  \
    if((c) && ctest_test_retval >= 0) {                                       \
...
// EXPECT_TRUE
#define EXPECT_TRUE(c) __TEST_TRUE(c, 1)

// EXPECT_FALSE
#define EXPECT_FALSE(c) __TEST_FALSE(c, 1)

// ASSERT_TRUE
#define ASSERT_TRUE(c) __TEST_TRUE(c, -1)

// ASSERT_FALSE
#define ASSERT_FALSE(c) __TEST_FALSE(c, -1)
...
#define __TEST_EQ(a, b, ret)                                                  \
    do {                                                                      \
      int64_t __a = (int64_t)(a);                                             \
      int64_t __b = (int64_t)(b);                                             \
      if(__a!=__b && ctest_test_retval >= 0) {                                \
        printf("ERROR at %s: %d, EXPECT_EQ("                                  \
             #a ", " #b ") (a=%" PRId64 ",b=%" PRId64 ")\n",                  \
             __FILE__, __LINE__, __a, __b);                                   \
        ctest_test_retval = (ret);                                            \
        if (ctest_test_retval < 0) {                                          \
          longjmp(ctest_jmp_buf, ctest_test_retval);                          \
        }                                                                     \
      }                                                                       \
    } while (0)

#define __TEST_NE(a, b, ret)                                                  \
    do {                                                                      \
      int64_t __a = (int64_t)(a);                                             \
      int64_t __b = (int64_t)(b);                                             \
      if(__a == __b && ctest_test_retval >= 0) {                              \
        printf("ERROR at %s: %d, EXPECT_NE("                                  \
               #a ", " #b ") (a=%" PRId64 ",b=%" PRId64 ")\n",                \
               __FILE__, __LINE__, __a, __b);                                 \
        ctest_test_retval = (ret);                                            \
        if (ctest_test_retval < 0) {                                          \
...
#define EXPECT_EQ(a, b) __TEST_EQ(a, b, 1)
#define EXPECT_NE(a, b) __TEST_NE(a, b, 1)
#define ASSERT_EQ(a, b) __TEST_EQ(a, b, -1)
#define ASSERT_NE(a, b) __TEST_NE(a, b, -1)
...
static inline void EXPECT_CTEST_EQ(const char *location, const A &a, const B &b, bool neq, int ret)
{
    if ((!((int64_t)(a) == (int64_t)(b)) ^ neq) && ctest_test_retval >= 0) {
...
static inline void EXPECT_CTEST_EQ(
    const char *location, const std::string &a, const std::string &b, bool neq, int ret)
{
    if ((!(a == b) ^ neq) && ctest_test_retval >= 0) {
        printf("ERROR at %s\na=%s\nb=%s\n",
...
static inline void EXPECT_CTEST_EQ(
    const char *location, const std::string &a, const char *const bcs, bool neq, int ret)
{
    EXPECT_CTEST_EQ(location, a, std::string(bcs), neq, ret);
}

static inline void EXPECT_CTEST_EQ(
    const char *location, const char *const acs, const std::string &b, bool neq, int ret)
{
    EXPECT_CTEST_EQ(location, std::string(acs), b, neq, ret);
}

static inline void EXPECT_CTEST_EQ(
    const char *location, const char *const acs, const char *const bcs, bool neq, int ret)
{
    EXPECT_CTEST_EQ(location, std::string(acs), std::string(bcs), neq, ret);
}
#endif
